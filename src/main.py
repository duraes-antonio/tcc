...
def gerar_lista_arquivos(
	...
	treino_fatia = treino_ord[:int(len(treino_ord) * porcent_treino)]

	validacao_ord = list(set(treino_ord) - set(treino_fatia))
...
def gerar_dados_adicionais(img_treino_path: str, img_path: str):
	...
	for i in range(1, 11):
		...
		img_treino = modificar_img(img_treino_path, grau_rotacao, 0, porcent_zoom)
...
def main():
	...
	tarefas: Coroutine = atualizar_imagens(
		...
		bacteria=True, covid=True, altura_largura=(500, 500)
	)
	...
	for arq_path in arquivos_path:
		if 'covid' in arq_path:
			continue
		gerar_dados_adicionais(
			arq_path,
			arq_path.replace('.png', '.jpeg').replace(pasta_anot, pasta_img)
		)
