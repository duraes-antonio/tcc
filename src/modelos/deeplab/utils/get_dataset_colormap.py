...
def create_pqr_label_colormap():
	colormap = np.zeros((_DATASET_MAX_ENTRIES[_PQR], 3), dtype=int)
	ind = np.arange(_DATASET_MAX_ENTRIES[_PQR], dtype=int)
...
def create_label_colormap(dataset=_PQR):
...
def label_to_color_image(label, dataset=_PQR):
