...
class Dataset(object):
	...
	def __init__(
			self, dataset_name, split_name, dataset_dir, batch_size, crop_size,
			min_resize_value=None, max_resize_value=None, resize_factor=None,
			min_scale_factor=1., max_scale_factor=1., scale_factor_step_size=0,
			model_variant=None, num_readers=1, is_training=False,
			should_shuffle=False, should_repeat=False
	):
		...
		Args:
			...
			model_variant: Model variant (string) for choosing how to mean-subtract
				the images. See feature_extractor.network_map for supported model variants.
		...
		if model_variant is None:
			tf.logging.warning(
				...
				'feature_extractor.network_map for supported model variants.'
			)
	...
	def get_one_shot_iterator(self):
		...
		dataset = (
			tf.data
				.TFRecordDataset(files, num_parallel_reads=self.num_readers)
