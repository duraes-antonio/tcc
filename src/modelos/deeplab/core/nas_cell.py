...
separable_conv2d_same = functools.partial(
	xception_utils.separable_conv2d_same, regularize_depthwise=True
)
...
class NASBaseCell(object):
	...
	def __init__(
			self, num_conv_filters, operations, used_hiddenstates,
		...
		Args:
			...
			operations: List of operations that are performed in the NASNet Cell in order.
			...
			batch_norm_fn: Function, batch norm function. Defaults to slim.batch_norm.
		"""
		...
		if 'separable' in operation:
			...
			for layer_num in range(num_layers):
				...
				net = separable_conv2d_same(
					net, filter_size, kernel_size, depth_multiplier=1, stride=stride,
