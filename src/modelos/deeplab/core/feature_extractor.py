...
def _mobilenet_v2(
		net, depth_multiplier, output_stride, conv_defs=None,
		divisible_by=None, reuse=None, scope=None, final_endpoint=None
):
...
def _mobilenet_v3(
	...
	with tf.variable_scope(
		...
		net, end_points = mobilenet_v3.mobilenet_base(
			net, depth_multiplier=depth_multiplier, conv_defs=conv_defs,
			output_stride=output_stride, final_endpoint=final_endpoint, scope=scope
		)
		return net, end_points


def mobilenet_v3_large_seg(
		net, depth_multiplier, output_stride, divisible_by=None,
		reuse=None, scope=None, final_endpoint=None
):
...
def mobilenet_edgetpu(
	...
	return _mobilenet_v3(
		net, depth_multiplier=depth_multiplier, output_stride=output_stride,
		divisible_by=8, conv_defs=conv_defs, reuse=reuse,
		scope=scope,  # the scope is 'MobilenetEdgeTPU'
		final_endpoint=_MOBILENET_EDGETPU
	)


def mobilenet_v3_small_seg(
		net, depth_multiplier, output_stride, divisible_by=None, reuse=None,
		scope=None, final_endpoint=None
):
	...
	return _mobilenet_v3(
		net, depth_multiplier=depth_multiplier, output_stride=output_stride,
		divisible_by=8, conv_defs=conv_defs, reuse=reuse, scope=scope,
...
def mobilenet_v2_arg_scope(
		is_training=True, weight_decay=0.00004, stddev=0.09, activation=tf.nn.relu6,
		bn_decay=0.997, bn_epsilon=None, bn_renorm=None
):
...
networks_to_feature_maps = {
	...
	'nas_pnasnet': {
		DECODER_END_POINTS: {4: ['Stem'], 8: ['Cell_3'], 16: ['Cell_7']},
	},
	'nas_hnasnet': {
		DECODER_END_POINTS: {4: ['Cell_2'], 8: ['Cell_5'], 16: ['Cell_7']},
	},
}
...
def extract_features(
		images, output_stride=8, multi_grid=None, depth_multiplier=1.0,
		divisible_by=None, final_endpoint=None, model_variant=None,
		weight_decay=0.0001, reuse=None, is_training=False,
		fine_tune_batch_norm=False, regularize_depthwise=False,
		preprocess_images=True, preprocessed_images_dtype=tf.float32,
		num_classes=None, global_pool=False, nas_architecture_options=None,
		nas_training_hyper_parameters=None, use_bounded_activation=False
):
	...
	if 'resnet' in model_variant:
		arg_scope = arg_scopes_map[model_variant](
			weight_decay=weight_decay, batch_norm_decay=0.95,
			batch_norm_epsilon=1e-5, batch_norm_scale=True
		)
		features, end_points = get_network(
			model_variant, preprocess_images, preprocessed_images_dtype, arg_scope
		)(
			inputs=images, num_classes=num_classes,
			is_training=(is_training and fine_tune_batch_norm),
			global_pool=global_pool, output_stride=output_stride,
			multi_grid=multi_grid, reuse=reuse, scope=name_scope[model_variant]
		)
	elif 'xception' in model_variant:
		arg_scope = arg_scopes_map[model_variant](
			weight_decay=weight_decay, batch_norm_decay=0.9997,
			batch_norm_epsilon=1e-3, batch_norm_scale=True,
		...
		features, end_points = get_network(
			...
			is_training=(is_training and fine_tune_batch_norm),
			global_pool=global_pool, output_stride=output_stride,
			regularize_depthwise=regularize_depthwise, multi_grid=multi_grid,
			reuse=reuse, scope=name_scope[model_variant]
		)
		...
		features, end_points = get_network(
			...
			inputs=images, depth_multiplier=depth_multiplier,
			divisible_by=divisible_by, output_stride=output_stride,
			reuse=reuse, scope=name_scope[model_variant],
			final_endpoint=final_endpoint
		)
	elif model_variant.startswith('nas'):
		arg_scope = arg_scopes_map[model_variant](
			weight_decay=weight_decay, batch_norm_decay=0.9997, batch_norm_epsilon=1e-3
		)
		features, end_points = get_network(
			model_variant, preprocess_images, preprocessed_images_dtype, arg_scope
		)(
...
def get_network(network_name, preprocess_images,
	...
	def network_fn(inputs, *args, **kwargs):
		with slim.arg_scope(arg_scope):
			return func(
				preprocess_function(inputs, preprocessed_images_dtype), *args, **kwargs
			)
