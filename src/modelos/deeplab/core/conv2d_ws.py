...
def conv2d(
		...
		data_format=None, rate=1, activation_fn=tf.nn.relu, normalizer_fn=None,
		normalizer_params=None, weights_initializer=contrib_layers.xavier_initializer(),
		weights_regularizer=None, biases_initializer=tf.zeros_initializer(),
		biases_regularizer=None, use_weight_standardization=False, reuse=None,
	...
	with tf.variable_scope(
		...
		param = 'channels_first' if data_format and data_format.startswith('NC') else 'channels_last'
		data_format = (param)
		layer = Conv2D(
			filters=num_outputs,
			kernel_size=kernel_size,
