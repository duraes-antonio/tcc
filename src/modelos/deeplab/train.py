...
def define_flags_train_default() -> Any:
	flags = tf.app.flags
	...
	# https://www.tensorflow.org/api_docs/python/tf/compat/v1/flags/DEFINE_enum
	...
	flags.DEFINE_integer('training_number_of_steps', 1000, 'The number of steps used for training')
	...
	# Dataset settings.
	flags.DEFINE_string('dataset', 'pqr', 'Name of the segmentation dataset.')
...
flags = define_flags_train_default()
	...
	with tf.Graph().as_default() as graph:
		...
		with tf.device(config.optimizer_device()):
			...
			if FLAGS.optimizer == 'momentum':
				...
				optimizer = tf.train.AdamOptimizer(FLAGS.adam_learning_rate, epsilon=FLAGS.adam_epsilon)
			else:
